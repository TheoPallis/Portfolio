	...
	if st.button(" Today's Workout ") :                          														
		...
		if st_button("Nope"!) : ex_cols = st.slider ('Number of exercises', max_value = 10) 							
			...
			for e in range (0, ex_cols) :  		  																		# Get names of exercises (number of exercises = slider)
				...
				for sets in range (0, set_number.max()) :																# Get number of reps for each set
...
#1) Test on streamlit
#2) If flow -> elif/else
#3) Does nope! loop back to original choice?
#4) Can you select bring it on after selecting nope
#5) Number of loops = highest set number -> NFX5
